...
app.post('/api/scrape', async (req, res) => {
  ...
  db.query(query, [task_name, url, media_source, topic, subtopic], async (err, result) => {
      ...
      const imagePath = path.resolve(__dirname,'..','public', 'assets', 'images','tasks', imageFilename);

      try {
          // Step 2: Download and resize the image
          const response = await axios.get(thumbnail_url, { responseType: 'arraybuffer' });
          ...
          db.query(updateQuery, [imagePath, taskId], (updateErr) => {
              if (updateErr) {
